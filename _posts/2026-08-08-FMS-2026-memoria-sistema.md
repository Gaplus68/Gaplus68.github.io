...
description: "Dalla KV cache ai trend emergenti: cronaca dalla fine dello storage passivo, e perché il codice deve cambiare"
image:
...
E c'è un piano tutto open source che merita due righe, perché la KV cache non è più terreno riservato ai colossi. **LMCache** — il layer aperto per estrarre, conservare e trasferire la cache tra engine di inferenza — è già adottato da progetti come Dynamo di NVIDIA, llm-d (Red Hat, IBM Research, Google) e AIBrix di ByteDance; **Mooncake** di Moonshot AI ha aperto la strada al serving KV-cache-centrico; vLLM e SGLang integrano prefix caching e routing consapevole della cache. E poi c'è il progetto che seguiamo con più simpatia: **DwarfStar (ds4)** di Salvatore Sanfilippo — sì, l'autore di Redis — un motore di inferenza volutamente minimale, scritto in C, che tratta la KV cache come *cittadina di prima classe del disco*: sessioni serializzate su SSD, indirizzate dal prefisso del prompt, riprendibili a freddo senza rifare il prefill. La stessa idea — il contesto come oggetto persistente e riusabile — la trovi nei keynote di Santa Clara e in un repository gestito da una persona sola, che gira su un MacBook o su un mini PC Strix Halo. Non a caso il terzo giorno di FMS c'era un panel intitolato *The Future of KV$*: la cache con il dollaro nel nome. Dove la comunità open source apre la strada, il mercato arriva subito dopo.
...
Su circa 260 sessioni di contenuto, un quarto ruota attorno a una sola frase: *il contesto non si trova più dove stava prima*.
...
Tanti trend, nessuna certezza. Molti punti sono proposte che giocano su campi diversi, altri sono in potenziale contrasto. Come si evolverà il settore? Nessuno, a Santa Clara, aveva la risposta definitiva.
...
- [DwarfStar (ds4), il motore di inferenza di antirez — GitHub](https://github.com/antirez/ds4)
